...
declare global {
    ...
    export namespace umg {
        ...
        export function extend(parentEntity: LuaTable<string, any>, definition: LuaTable<String, unknown>): any;

        export function group<T>(...args: (keyof T)[]): Group<T>;

        export function exists(ent: unknown): ent is Entity;

        export function save(filename: string, data: string): void;
    ...
    export interface Entity {
        readonly id: number;
        isRegularComponent(componentName: string): boolean;
        isSharedComponent(componentName: string): boolean;
