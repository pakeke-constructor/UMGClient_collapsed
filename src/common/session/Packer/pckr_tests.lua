...
local function truthy() return true end
local function falsey() return false end

local function newDummyPckrState()
    local pState = newPckrState({
        canSerializeEntity = truthy,
        getEntityById = truthy, -- we aint testing this, dont matter
        deserializeEntity = truthy,
        shouldSerializeEntityById = falsey,
...
--[[
    dummy pckr state for testing ONLY
]]
local pckrState = newDummyPckrState()
...
local function check(tab, loud)
    local dat = pckrState:serialize(tab)
    local res, e = pckrState:deserialize(dat)
    if not res then
    ...
    if not r then
        ...
        error("pckrState:tests: CHECK FAILED")
...
pckrState:register(A, 1)
pckrState:register(B, "I am the wonderful B!!!!!")
local test1 = {A,B,C,D}
check(test1)
end
...
do
local bigdat = {{{{{{{{{}}}}}}}}}
pckrState:register(bigdat, "a")
local d = pckrState:serialize(bigdat)
assert(#d < 7, ".register not working") -- compresseion should take about 5 bytes
pckrState = newDummyPckrState()
local d2 = pckrState:serialize(bigdat)
...
do
pckrState:register(math, "abcdef")
check({math, 1, math})
pckrState = newDummyPckrState()
end
...
log.info("[pckr]: all tests passed")
